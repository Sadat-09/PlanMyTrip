    ...
    <table border="1">
        ...
        <?php for ($i = 0; $i < count($results); $i++) { ?>
            <tr>
                ...
                <input type="hidden" id="hotelname" name="hotelname[]" value="<?=$results[$i]['hotel_name']?>"></td>
                <td><?=$results[$i]['room_type']?>
                <input type="hidden" id="roomtype" name="roomtype[]" value="<?=$results[$i]['room_type']?>"></td>
                <td><?=$results[$i]['location']?>
                <input type="hidden" id="location" name="location[]" value="<?=$results[$i]['location']?>"></td>
                <td><?=$results[$i]['date']?>
                <input type="hidden" id="date" name="date[]" value="<?=$results[$i]['date']?>"></td>
                <input type="hidden" id="idd" name="id[]" value="<?=$results[$i]['room_id']?>"></td>
                <td><?= $_SESSION['amount'] = $results[$i]['Price']?></td>
                <td><input type="button" name="submit" value="Book" onclick="Booking(<?=$i?>)"></td>
            </tr>
        <?php } ?>
    </table>
