        ...
        <?php for ($i = 0; $i < count($results); $i++) { ?>
            <form action="CancelBooking.php" method="post">
                <tr>
                    ...
                    <input type="hidden" name="hotelname" value="<?=$results[$i]['hotel_name']?>"></td>
                    <td><?=$results[$i]['room_type']?>
                    <input type="hidden" name="room_type" value="<?=$results[$i]['room_type']?>"></td>
                    <td><?=$results[$i]['location']?>
                    <input type="hidden" name="location" value="<?=$results[$i]['location']?>"></td>
                    <td><?=$results[$i]['date']?>
                    <input type="hidden" name="date" value="<?=$results[$i]['date']?>"></td>
                    <input type="hidden" name="id" value="<?=$results[$i]['Id']?>">
                    <td><input type="submit" name="submit" value="Cancel Book"></td>
                </tr>
            </form>
        <?php } ?>
    </table>
